...
- Unit installer: `./scripts/install_systemd_units.sh --dry-run --environment <production|staging> --source-ref <approved-40-hex-sha>` from that environment's fixed, clean checkout. It stages the exact environment's five committed units and digest manifest in a private root-owned directory before preflight; production and staging refs are independent.
...
The deploy helper always refuses dirty worktrees. Dry-run privately materializes the exact remote candidate and executes read-only runtime/database, dependency resolver, and Alembic-head checks. Actual deploy creates `.runtime-deploying` before source mutation; every runtime service refuses starts under that guard. It atomically writes the selected clean remote commit's full SHA to `.runtime-release.env` before running new-tree preflight, pip, or Alembic. Later failures preserve aligned code/identity and leave the guard present; post-restart failure also stops the backend. There is no identity-only rollback or destructive Git reset. It accepts readiness only when parsed JSON reports exact `status=ok`, `db=ok`, environment, and full release SHA. The app consumes this artifact from systemd and never invokes Git. Install the guard-aware unit bundle before relying on this contract. Use `./scripts/deploy.sh --dry-run` before real deploys. There is no dirty-deploy override.

Integration gate: leave `rh_20260720_runtime_metadata` directly after `pa_20260715_analytics_facts`; the combined tree reparents `sec_20260720_identity` onto `rh` and keeps security linear through `sec_20260720_device`. Combined immutable unit allowlists must add both environments' auth-maintenance service/timer pairs. Do not retain local-monitor's identity-only failure rollback, which would pair stale SHA with new bytes.
