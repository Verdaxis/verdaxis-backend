...
The Negotiations subsystem handles **5 routes** and touches: auth, db.

## Routes

- `POST` `` → out: NegotiationResponse [auth, db]
  `app/routers/negotiations.py`
- `GET` `/{negotiation_id}` params(negotiation_id) → in: Annotated, out: NegotiationResponse [auth, db]
  `app/routers/negotiations.py`
