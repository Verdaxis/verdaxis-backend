...
async def publish_inventory_item(
    ...
    """
    Backward-compatible endpoint for marketplace tests.
    Converts an inventory item into an ASK listing on the unified orderbook.
    """
    if current_user.role != UserRole.SUPPLIER:
...
async def list_direct_orders(
    ...
    """
    Backward-compatible endpoint exposing de-anonymized trade counterparts.
    """
    if not current_user.organization_id:
        return []
    ...
    stmt = (
        ...
        .where(
            or_(
                Trade.buyer_id == current_user.organization_id,
                Trade.seller_id == current_user.organization_id,
            )
        )
    ...
    for trade in trades:
        response.append(
            {
                "id": str(trade.id),
                "supplier_name": trade.seller.name if trade.seller and trade.seller.name else "Unknown",
                "buyer_name": trade.buyer.name if trade.buyer and trade.buyer.name else "Unknown",
