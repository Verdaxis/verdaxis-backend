...
app/
  ...
  models/
    ...
    user_activity.py            # Authenticated browsing events, per-user UUID deduplication, 90-day retention
  ...
  routers/
    ...
    user_activity.py            # Authenticated browsing intake and admin per-user timeline
  ...
  services/
    ...
    user_activity.py            # Paginated browsing/business/login timeline with safe audit projections
...
deploy/external_monitor/         # Public/recovery monitoring plus production onboarding-attention timer
...
- **Account approval email:** The canonical administrator account-approval route persists the exact pending status-transition UUID, immutable provider payload, and due time with account status and audit state. After commit, delivery briefly locks and revalidates the user, then commits a five-minute claim in the existing retry timestamp before provider I/O. It replays the frozen payload with the transition UUID as Resend's idempotency key, then re-locks and finalizes only the same transition and claim. A crashed worker leaves a retryable claim; failures move the due time forward by one hour so one bad recipient cannot starve the queue. Rejection invalidates pending approval mail without waiting for an in-flight send, whose completion cannot clear a newer transition. Idempotent re-approval does not enqueue again, and email failure cannot roll back admission. Organization, membership, KYC, and trading-access approvals remain independent gates.
...
- **Per-user activity:** Authenticated batches store only allowlisted page and canonical market fields at server receipt time, deduplicated by `user_id + event_id`. The deprecated optional consent marker is retained only for rolling compatibility; new rows store `NULL`. Privacy terms are maintained outside this application. The admin timeline SQL-unions retained browsing, daily login aggregates, safe business-audit projections, and current pre-audit watchlist/preference facts. It never exposes audit IP, request IDs, or unlisted changes. The existing product-analytics prune timer deletes browsing rows older than 90 days; durable audit history is unchanged.
- **Live runtime topology:** Production systemd binds Uvicorn to `127.0.0.1:8000`; staging binds to `127.0.0.1:8001`; the public reverse proxy fronts those loopback ports. Both services require network-online and PostgreSQL, use four workers, and preflight the exact migration checkpoint published with the release rather than a later source head. News refresh has exactly one external owner per environment: the environment-specific six-hourly systemd timer invokes `app.cli.refresh_news`, while a PostgreSQL transaction advisory lock rejects overlap. Production prefers one bounded licensed Webz.io news request and falls back to maritime RSS; staging normally uses RSS only. Open-web news never enters trusted market-signal or benchmark ingestion. Application workers create no scheduler and the public API exposes no manual refresh mutation. Product-analytics pruning likewise has one timer owner per environment; it requires PostgreSQL/network readiness, uses bounded boot retries and the news sandbox, and must match that environment's `.runtime-release.env` identity before database access. Every runtime service checks the durable `.runtime-deploy/<environment>.state` and refuses blocked/readiness-pending starts.
...
- **External recovery and incident diagnosis:** `deploy/external_monitor` tracks the legacy public monitor. Its first `OnFailure` stage may make one allowlisted recovery attempt per failure fingerprint per hour: restart one backend only outside deploy transactions, validated Caddy reload/restart, or the existing backup service with disk headroom. It then reruns the full public monitor; unknown, failed, or suppressed recovery proceeds to read-only Codex diagnosis. The diagnosis wrapper exits if the current monitor status is healthy, atomically synchronizes the current owner-only central Codex credential into its isolated Codex home, limits shared Caddy repository evidence to Verdaxis-owned paths, includes the bounded recovery report, redacts monitor/service evidence, removes alert credentials from the Codex child environment, and invokes an ephemeral read-only Codex session with a strict output schema. Codex cannot edit, restart, deploy, commit, push, or mutate production. An independent VPS checks critical public routes every five minutes and can invoke only the monitor through a forced SSH command; provider-level host/network outages remain outside application recovery authority.
