...
- **External incident diagnosis:** `deploy/external_monitor` tracks the legacy public monitor and its systemd `OnFailure` diagnosis path. A repeated failure fingerprint is suppressed for one hour. The diagnosis wrapper redacts and bounds monitor/service evidence, removes alert credentials from the Codex child environment, and invokes an ephemeral read-only Codex session with a strict output schema. It can report findings through Telegram but cannot edit, restart, deploy, commit, push, or mutate production. Because it runs on the Verdaxis VPS, whole-host outages still require an independently hosted trigger.
