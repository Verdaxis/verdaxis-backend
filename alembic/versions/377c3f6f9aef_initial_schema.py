...
def upgrade() -> None:
    ...
    # Enums
    user_role = postgresql.ENUM('BUYER', 'SUPPLIER', 'ADMIN', name='user_role', create_type=False)
    org_type = postgresql.ENUM('SHIPPING_LINE', 'FUEL_SUPPLIER', 'PORT_AUTHORITY', name='org_type', create_type=False)
    congestion_level = postgresql.ENUM('Low', 'Moderate', 'High', name='congestion_level', create_type=False)
    fuel_type = postgresql.ENUM('Methanol', 'Biofuel', 'LNG', 'Ammonia', 'LSMGO', name='fuel_type', create_type=False)
    quote_status = postgresql.ENUM('Draft', 'Pending', 'Quoted', 'Negotiating', 'Confirmed', 'Rejected', 'Completed', name='quote_status', create_type=False)
    
    # Actually create them once manually using SQL (IF NOT EXISTS logic)
